...
| Domain | Tool | Architectural Decision |
|---|---|---|
...
1. **Infrastructure (cAdvisor):** Tracks raw Network I/O and CPU throttling at the container level.
2. **Edge (NGINX):** Tracks HTTP 500 errors and Latency at the Ingress controller.
3. **Visuals:** Custom Grafana dashboards built with PromQL to aggregate `rate()` of request volume per namespace.
